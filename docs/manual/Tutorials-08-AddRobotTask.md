...
- Go to `engineai_gym/resources/robots/{robot type}` and create a folder named `{robot_name}` with URDF, mesh and other related files (e.g. mocap_dataset)
- Go to `engineai_gym/engineai_gym/envs/robots/{robot type}/{robot}` to add env, obs, goals, reward, domain_rands and config
...
class Anymal(LeggedRobot):
    ...
    def __init__(self, reward_class, cfg, sim_params, physics_engine, sim_device, headless):
        super().__init__(reward_class, cfg, sim_params, physics_engine, sim_device, headless)

        # load actuator network
        if self.cfg.control.use_actuator_network:
            actuator_network_path = self.cfg.control.actuator_net_file.format(ENGINEAI_GYM_ROOT_DIR=ENGINEAI_GYM_ROOT_DIR)
...
Go to `engineai_gym/engineai_gym/envs/robots/{robot type}/{robot}` to create the task folder and add the logic and config of the task , which inherits from robot
...
class ConfigPm01Rough(ConfigBipedRobot):
    ...
    class domain_rand(ConfigBipedRobot.domain_rand):
        class rigid_shape(ConfigBipedRobot.domain_rand.rigid_shape):
            randomize_friction = True
            friction_range = [0.2, 1.3]
            randomize_restitution = True
            restitution_range = [0.0, 0.4]

        class rigid_body(ConfigBipedRobot.domain_rand.rigid_body):
        ...
        class dof(ConfigBipedRobot.domain_rand.dof):
            ...
            joint_friction_multiplier_range_each_joint = {
                "hip": [0.01, 1.15],
                "knee": [0.01, 1.15],
                "ankle": [0.5, 1.3],
            }
            ...
            joint_armature_multiplier_range = [0.27, 2]  # multiplier
            joint_armature_multiplier_range_each_joint = {
                "hip": [0.27, 2],
                "knee": [0.27, 2],
                "ankle": [0.27, 2],
            }
        ...
        class action_lag(ConfigBipedRobot.domain_rand.action_lag):
        ...
        class obs_lag(ConfigBipedRobot.domain_rand.obs_lag):
        ...
        class disturbance(ConfigBipedRobot.domain_rand.disturbance):
            push_robots = True
            push_interval_s = 8
            max_push_vel_xy = 0.4
